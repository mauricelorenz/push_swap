...
/*   Updated: 2026/01/08 11:23:48 by mlorenz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../push_swap_bonus.h"

static int	execute_instruction(t_stack **stack_a, t_stack **stack_b, char *instruction);
...
int	check_sorting(t_stack **stack_a, t_stack **stack_b)
{
	...
	{
		// char *to_print = (char *)instruction_ptr->content;
		// while (*to_print)
		// {
		// 	write(1, to_print, 1);
		// 	to_print++;
		// }
		if (!execute_instruction(stack_a, stack_b, (char *)instruction_ptr->content))
			return(ft_lstclear(&instruction_list, delete_instruction), ft_putendl_fd("Error", 2), 2);
		instruction_ptr = instruction_ptr->next;
	}
	if (is_sorted(stack_a) && !(*stack_b))
		return (ft_lstclear(&instruction_list, delete_instruction), ft_putendl_fd("OK", 1), 0);
	return (ft_lstclear(&instruction_list, delete_instruction), ft_putendl_fd("KO", 1), 1);
}
